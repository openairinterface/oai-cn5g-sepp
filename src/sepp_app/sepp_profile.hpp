...
#ifndef FILE_SEPP_PROFILE_HPP_SEEN
#define FILE_SEPP_PROFILE_HPP_SEEN
...
public:
  nf_profile()
      : nf_type("NF_TYPE_UNKNOWN"), heartBeat_timer(0), snssais(), fqdn(),
        ipv4_addresses(), ipv6_addresses(), priority(0), capacity(0) {
    nf_instance_name = "";
    nf_status = "";
  }

  nf_profile(const std::string &id)
      : nf_instance_id(id), nf_type("NF_TYPE_UNKNOWN"), heartBeat_timer(0),
        snssais(), fqdn(), ipv4_addresses(), ipv6_addresses(), priority(0),
        capacity(0) {
    nf_instance_name = "";
    nf_status = "";
  }
  ...
  /*
   * Covert from a json represetation to SEPP profile
   * @param [nlohmann::json &] data: Json data
   * @return void
   */
...
class sepp_profile : public nf_profile {
public:
  sepp_profile() : nf_profile() { custom_info = {}; }

  sepp_profile(const std::string &id) : nf_profile(id) { custom_info = {}; }

  sepp_profile &operator=(const sepp_profile &s) {
    nf_instance_id = s.nf_instance_id;
    heartBeat_timer = s.heartBeat_timer;
    snssais = s.snssais;
    ipv6_addresses = s.ipv6_addresses;
    ...
    sepp_info = s.sepp_info;
    nf_services = s.nf_services;
    return *this;
  }
  // sepp_profile(sepp_profile &b) = delete;

  virtual ~sepp_profile() {
    Logger::sepp_app().debug("Delete SEPP Profile instance...");
  }
  ...
  /*
   * Set sepp info
   * @param [sepp_info_t &] s: sepp info
   * @return void
   */
  void set_sepp_info(const oai::common::sbi::sepp_info_t &s);

  /*
   * Add an snssai_sepp_info_item to the sepp info
   * @param [const snssai_sepp_info_item_t &] s: snssai_sepp_info_item
   * @return void
   */
  // void add_sepp_info_item(const snssai_sepp_info_item_t& s);

  /*
   * Get NF instance sepp info
   * @param [sepp_info_t &] s: store instance's sepp info
   * @return void:
   */
  void get_sepp_info(oai::common::sbi::sepp_info_t &s) const;
...
protected:
  ...
  oai::common::sbi::sepp_info_t sepp_info;
};
