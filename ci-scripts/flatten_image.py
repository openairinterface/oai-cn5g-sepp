...
def perform_flattening(tag):
    ...
    if cli == 'docker':
      ...
      cmd += ' --change "ENV PATH=/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin" '
    cmd += ' --change "WORKDIR /openair-sepp" '
    cmd += ' --change "EXPOSE 80/tcp" '
    cmd += ' --change "EXPOSE 8080/tcp" '
    cmd += ' --change "HEALTHCHECK --interval=10s --timeout=15s --retries=6 CMD /openair-sepp/bin/healthcheck.sh" '
    cmd += ' --change "CMD [\\"/openair-sepp/bin/oai-sepp\\", \\"-c\\", \\"/openair-sepp/etc/config.yaml\\", \\"-o\\"]" '
